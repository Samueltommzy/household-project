...
public class Family
{
    ...
    //Add child to a specific position
    public void addChild(String name,int age,int position){
        //check that current child count does not exceed position
        if(position < childCount){
            for(int i = childCount;i>position-1;i--){
...
//        childre
    ...
    public void removeAChild(int position){
        if(position > 0&&position <= childCount){
            for(int j = position;j<childCount;j++){
